...
async function ensureGitHubSources(): Promise<void> {
  const projects = await prisma.project.findMany({
    where: { isAutoGenerated: true },
    select: {
      id: true,
      keywords: true,
      sources: { where: { platform: 'GITHUB' }, select: { id: true } },
    },
  });

  for (const project of projects) {
    if (project.keywords.length === 0) continue;
    if (project.sources.length > 0) continue; // already has a GitHub source

    // Use up to 3 keywords; quote multi-word terms
    ...
    logger.info({ projectId: project.id, query: terms }, 'Auto-created GitHub search source');
  }
}

async function handleScrapeAll(): Promise<void> {
  await ensureGitHubSources();
